...
기술 지표 해설 규칙 (엄수):
- 이동평균선·RSI·MACD·볼린저밴드·스토캐스틱을 지표별로 나열하지 말고, 모든 지표를 종합하여 현재 추세·모멘텀·변동성·매매 시그널을 **총 4문장 이내**로 서술하세요.
- 지표명은 언급 가능하나 "RSI는 ...이고, MACD는 ...이며, 볼린저밴드는 ..." 같은 지표별 나열식 서술은 금지합니다.
- 각 지표의 정의·공식·해석 방법을 설명하지 마세요 (사용자는 이미 지표를 알고 있음).
        ...
        try:
            ...
            async with client.messages.stream(**call_kwargs) as stream:
                ...
                try:
                    ...
                    logger.info(
                        "Claude 토큰 사용: input=%s, output=%s, cache_read=%s, cache_create=%s, ticker=%s",
                    ...
                    logger.warning("토큰 사용량 로깅 실패: %s", log_err)
